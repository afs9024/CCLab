...
function preload() {
  for (let i = 1; i <= 5; i++) {
    scanned.push(loadImage("20260320104754333_000" + i + ".jpg"));
  }
}
...
function setup() {
  ...
  car1 = crop(scanned, 1514, 1300, 830, 300);
  car2 = crop(scanned, 0, 1400, 266, 300);
}
...
function draw() {
  ...
  curDoodle1 = floor(map(sin(frameCount / 10), -1, 1, 0, car1.length));
...
curDoodle2 = floor(map(sin(frameCount / 5), -1, 1, 0, car2.length));
 ...
 if (mouseIsPressed == true) {
  ...
  let d = dist(mouseX, mouseY, 0, 100);
  if (d < 100) {
